...
- (void)draw {
    ...
    CGSize labelSize = CGSizeMake(self.barWidth, 50);
    ...
    for (int i = 0; i < self.xValues.count; i++) {
        ...
        CGPoint center = CGPointMake((i+1) * self.barGap + i * self.barWidth + 8, self.chartHeight - (value.floatValue / self.yAxisMaxValue) * self.chartHeight + self.topMargin );
        
        if (self.yAxisMaxValue * self.chartHeight == 0) {
            center = CGPointMake((i+1) * self.barGap + i * self.barWidth + 8, self.chartHeight + self.topMargin);
        }
        ...
        button.x = center.x;
        button.y = center.y + (button.size.height / 2);
